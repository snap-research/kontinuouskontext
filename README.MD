...
## [**arXiv**](https://arxiv.org/abs/2510.08532) · [Project Page](https://snapresearch.github.io/kontinuouskontext/) · [Demo](https://huggingface.co/spaces/snap-research/KontinuousKontext) 
...
- [ ] Generated Dataset for model training(full training set + metadata)
...
We build our model on Flux Kontext (link), an instruction based image editing model. For training you have to download the Flux Kontext model and the training dataset as discussed below.
...
You can start a training run by executing the `run_modulaiton_train.sh` script. The base `FLUX.1-Kontext-dev` model will be downloaded automatically by the script on the first run.

```bash
bash run_modulation_train.sh
```

Your training logs will be saved inside the ./logs/ folder.

## 4. Training Script (run_modulaiton_train.sh)
